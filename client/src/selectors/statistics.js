...
export const getOperations = createSelector(selectOperations, propOr([], 'items'));

export const getDate = createSelector(selectOperations, propOr({}, 'date'));

export const getTypeSwitch = createSelector(selectOperations, propOr({}, 'type'));

export const getGroupByYears = createSelector(getOperations, (items) => {
  const years = [];
  items.forEach((item) => {
...
export const getCurrentDatesObject = createSelector(getDate, (date) => {
  const currentDates = {
    day: dayjs(date).date(),
...
export const getValuesByType = createSelector([getCurrentDatesObject, getOperations, getTypeSwitch, getDate], (currentDates, items, type, date) => {
  const stats = {
    income: 0,
    expense: 0,
  };
