...
export const getOperationList = createSelector(selectOperations, propOr([], 'items'));

export const getOperationByDate = createSelector(getOperationList, (items) => items.sort((a, b) => (new Date(b.date) < new Date(a.date) ? -1 : 1)));

export const getGroupsByDate = createSelector(getOperationByDate, (items) => {
...
export const getDate = createSelector(selectDate, propOr({}, 'date'));

export const getMonth = createSelector(getDate, (date) => {
...
export const getValuesByMonth = createSelector([getMonth, getOperationList], (currentDatesObject, items) => {
  const stats = {
    income: 0,
    expense: 0,
  };
