...
export default class StatisticsPage extends React.Component {
  ...
  makeChart = () => {
    const data = {
      labels: this.createGroups().map((item) => {
        return item.label;
      }),
      datasets: [
        {
          data: this.createGroups().map(({ transactions }) => this.categoriesValues(transactions)),
  ...
  switchCalendars = () => {
    ...
    if (type === 'год') {
      ...
      return (
        <Dropdown placeholder={`${ formatYear }`} value={yearNow} options={years} className={cx('year')} onChange={(e) => onChangeDate(e.value)} />
      );
    }

    if (type === 'все время') return null;

    if (type === 'интервал') return (
      <Calendar
        inputStyle={{
          border: 'none',
          backgroundColor: '#ffffff',
  ...
  createGroups = () => {
    ...
    const groupsArray = [];
    groups.forEach((transactions, label) => groupsArray.push({ transactions, label }));
    return groupsArray;
  };
      ...
      <div className={cx('body')}>
        <div className={cx('control-panel')}>
          <div className={cx('controls')}>
            ...
            />
            <div className={cx('calendar')}>{this.switchCalendars()}</div>
          </div>
        </div>
        <div className={cx('preview')}>
          <div className={cx('title')} key={Date.now()}>
            <div className={cx('title-bar')}>
              ...
              <ProgressBar value={stats.income - stats.expense} className={cx('ttl-bar')} showValue={false} />
            ...
            <div className={cx('title-bar')}>
              ...
              <ProgressBar value={stats.income} className={cx('ttl-bar', 'incomes')} showValue={false} />
              <div className={cx('value')}>
            ...
            <div className={cx('title-bar')}>
              ...
              <ProgressBar value={stats.expense} className={cx('ttl-bar', 'expense')} showValue={false} />
              <div className={cx('value')}>
        ...
        {this.createGroups().map(({ transactions, label }, key) => (
          <StatisticsPageProgressBars
            ...
            value={this.categoriesValues(transactions)}
          />
        ))}
