...
export default class PageIncExp extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (categories !== prevProps.categories) this.setState({
      categories,
    });
  }
  ...
  filterCategories = (event) => {
    ...
    if (event.query.length === 0) results = [...this.state.categories];
    else results = this.state.categories.filter((category) => {
  ...
  categoriesTemplate = (category) => {
    return <div style={{ fontSize: '16px' }}>{category.title}</div>;
  };
            ...
            <AutoComplete
              ...
              onChange={(e) => {
                if (typeof e.value === 'string') {
                  const category = this.state.categories.find((category) => category.title.toLowerCase() === e.value.toLowerCase()) || {
                    title: e.value,
                  };
                  this.setState({ selectedCategory: category });
                } else this.setState({ selectedCategory: e.value });
              }}
            />
          </div>
          <div className={cx('incexp-body-button')}>
            <div className={cx('buttons-container')}>
              <Button label="Доход" className="p-button-secondary" onClick={() => this.onButtonClick('income')} />
              <Button label="Расход" className="p-button-secondary" onClick={() => this.onButtonClick('expense')} />
            </div>
          </div>
        </div>
      </div>
